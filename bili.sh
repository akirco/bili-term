...
# 版本常量
readonly BILI_TERM_VERSION="v2.0.0"
...
fetch_personal_recommend() {
	_fetch_recommend "${PERSONAL_PAGE_SIZE:-20}"
}
...
do_login() {
	...
	while [ $poll_count -lt $max_poll ]; do
		...
		0)
			...
			if check_login; then return 0; else
...
run_main_loop() {
	while true; do
		...
		*"$MENU_PERSONAL_RECOMMEND"*) run_fzf_video_list "personal" ;;
